...
__q3__ Notice that the `trans` column of `mpg` has many entries of the form `auto|manual\\([:alpha:]\\d\\)`; use `str_mutate()` to create a new column `tmp` with just the code inside the parentheses extracting `[:alpha:]\\d`.
